...
// ============================================================
// 1. RATE LIMITING В SUPABASE (с расширенным логированием)
// ============================================================
...
async function checkRateLimit(ip) {
    ...
    console.log(`[rate-limit] === НОВЫЙ ЗАПРОС ===`);
    console.log(`[rate-limit] IP: ${ip}`);
    console.log(`[rate-limit] Время: ${now.toISOString()}`);

    try {
        const url = `${SUPABASE_URL}/rest/v1/rate_limits?ip=eq.${ip}`;
        console.log(`[rate-limit] Запрос к Supabase: ${url}`);
        ...
        console.log(`[rate-limit] Ответ от Supabase:`, JSON.stringify(data, null, 2));

        if (!data || data.length === 0) {
            console.log(`[rate-limit] IP новый, создаём запись`);
            await fetch(`${SUPABASE_URL}/rest/v1/rate_limits`, {
                ...
                body: JSON.stringify({ ip, count: 1, first_request: now.toISOString() })
            });
            console.log(`[rate-limit] Запись создана, разрешено`);
            return { allowed: true };
        }

        const record = data[0];
        const firstReq = new Date(record.first_request);
        const diff = now - firstReq;

        console.log(`[rate-limit] Текущий счёт: ${record.count}`);
        console.log(`[rate-limit] Первый запрос: ${firstReq.toISOString()}`);
        console.log(`[rate-limit] Разница: ${diff} мс (${diff/1000} сек)`);

        if (diff > windowMs) {
            console.log(`[rate-limit] Окно истекло (${diff} > ${windowMs}), сбрасываем счётчик`);
            await fetch(`${SUPABASE_URL}/rest/v1/rate_limits?ip=eq.${ip}`, {
                method: 'PATCH',
                headers: {
                    'apikey': SUPABASE_ANON_KEY,
                    'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                ...
                body: JSON.stringify({ count: 1, first_request: now.toISOString() })
            });
            console.log(`[rate-limit] Счётчик сброшен, разрешено`);
            return { allowed: true };
        }

        if (record.count >= limit) {
            console.log(`[rate-limit] ❌ ЛИМИТ ПРЕВЫШЕН! Счёт: ${record.count}, лимит: ${limit}`);
            return { allowed: false, reason: `Слишком много запросов (макс. ${limit} в минуту). Подождите.` };
        }

        const newCount = record.count + 1;
        console.log(`[rate-limit] Увеличиваем счётчик до ${newCount}`);
        await fetch(`${SUPABASE_URL}/rest/v1/rate_limits?ip=eq.${ip}`, {
            method: 'PATCH',
            headers: {
                'apikey': SUPABASE_ANON_KEY,
        ...
        console.log(`[rate-limit] Счётчик обновлён, разрешено`);
        return { allowed: true };
    } catch (error) {
        console.error('[rate-limit] ❌ Ошибка:', error);
        return { allowed: true };
    }
}
...
// --- Список нецензурных слов (оригинал из твоего файла) ---
const BAD_WORDS = [
...
app.post('/api/graves', async (req, res) => {
    // Исправление: берём первый IP из цепочки x-forwarded-for
    const forwarded = req.headers['x-forwarded-for'];
    const ip = forwarded ? forwarded.split(',')[0].trim() : req.socket.remoteAddress || 'unknown';
    console.log(`[POST /api/graves] Запрос от IP: ${ip}`);

    // 1. Rate Limiting
    const rateCheck = await checkRateLimit(ip);
    console.log(`[POST /api/graves] Результат rate limiting:`, rateCheck);
    if (!rateCheck.allowed) {
        console.log(`[POST /api/graves] ❌ Отказ из-за лимита`);
    ...
    try {
        ...
        const data = await response.json();
        console.log(`[POST /api/graves] ✅ Могила создана, ID: ${data[0]?.id || 'неизвестен'}`);
        res.json(data[0]);
    } catch (error) {
        console.error('[POST /api/graves] ❌ Ошибка сохранения:', error);
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/comments', async (req, res) => {
    ...
    const forwarded = req.headers['x-forwarded-for'];
    const ip = forwarded ? forwarded.split(',')[0].trim() : req.socket.remoteAddress || 'unknown';
    console.log(`[POST /api/comments] Запрос от IP: ${ip}`);

    // Rate Limiting
    const rateCheck = await checkRateLimit(ip);
    if (!rateCheck.allowed) {
        console.log(`[POST /api/comments] ❌ Отказ из-за лимита`);
        return res.status(429).json({ error: rateCheck.reason });
    }
    ...
    try {
        ...
        const data = await response.json();
        console.log(`[POST /api/comments] ✅ Комментарий создан`);
        res.json(data[0]);
    } catch (error) {
        console.error('[POST /api/comments] ❌ Ошибка сохранения:', error);
        res.status(500).json({ error: error.message });
    }
});
